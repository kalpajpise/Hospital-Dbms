<form action="" method="post">
                ...
                <?php 
                    if (isset($_POST['room_submit'])) {
                         $query = " SELECT * FROM employee e , consults c, patient p , room ro NATURAL JOIN room_patient rp where p.p_id = rp.p_id  AND e.e_id = c.e_id AND c.p_id=p.p_id AND ro.r_name = '$room_name' AND mask_del_pat = 0  ORDER BY p.p_id DESC ";
                        ...
                        while($row = mysqli_fetch_assoc($select_patient)) {
                            ...
                            echo "<td><a href='patient.php?source=detail_pat&edit={$pat_id}'>Details</a></td>";
                            echo "</tr>";
                            ...
                            $query = "SELECT * FROM employee e , consults c, patient p , room ro NATURAL JOIN room_patient rp where p.p_id = rp.p_id  AND e.e_id = c.e_id AND c.p_id=p.p_id AND mask_del_pat = 0 ORDER BY p.p_id DESC ";
                            $select_patient = mysqli_query($connection,$query); 
                            confirmQuery($select_patient);
